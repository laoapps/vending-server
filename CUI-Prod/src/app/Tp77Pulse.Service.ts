...
export class Tp77PulseService implements ISerialService {
  ...
  private pulseTimeout = 1000; // Timeout to finalize pulse sequence
  private pulseDebounce = 50;  // Minimum time (ms) between pulses to avoid noise
  ...
  // DIP switch settings for pulses per 20 THB bill
  private pulsesPerBillOptions = [
    15, 30, 45, 60, 75, 150, 300, 1500
  ];
  private pulsesPerBill = this.pulsesPerBillOptions[0]; // Default: 15 pulses = 20 THB
  private billValue = 20; // Fixed 20 THB per bill
  ...
  private initTp77(): void {
    this.getSerialEvents().subscribe((event) => {
      if (event.event === 'dataReceived') {
        ...
        this.addLogMessage(this.log, `Serial error: ${event.data}`);
      }
    });

    // Pulse sequence timeout handler
  ...
  private processPulseData(hexData: string): void {
    // Assuming TP77 sends '01' for a pulse high and '00' for low over serial
    // Adjust this if TP77 uses a different signal format
    for (let i = 0; i < hexData.length; i += 2) {
      const byte = hexData.slice(i, i + 2).toLowerCase();
      if (byte === '01' && Date.now() - this.lastPulseTime >= this.pulseDebounce) {
        this.pulseCount++;
  ...
  private processBill(): void {
    if (this.pulseCount >= this.pulsesPerBill) {
      ...
      this.pulseCount = this.pulseCount % this.pulsesPerBill; // Retain remainder
    } else if (this.pulseCount > 0) {
      this.addLogMessage(this.log, `Incomplete pulse sequence: ${this.pulseCount}/${this.pulsesPerBill} pulses`);
      this.pulseCount = 0; // Reset on incomplete sequence
    }
    this.lastPulseTime = Date.now();
  }
    ...
    return new Promise<string>(async (resolve, reject) => {
      ...
      try {
        ...
        if (init === this.portName) {
          ...
          reject(`Failed to open port: ${init}`);
        }
      } catch (error) {
        reject(`Serial port initialization failed: ${error}`);
      }
    });
  }
    ...
    return new Promise<IResModel>((resolve) => {
      switch (command) {
        ...
        case EMACHINE_COMMAND.SET_PULSE_COUNT:
          if (params?.pulseCount && typeof params.pulseCount === 'number') {
            this.setPulsesPerBill(params.pulseCount);
            resolve(PrintSucceeded(command, { pulsesPerBill: this.pulsesPerBill }, 'pulse count set'));
          } else {
            resolve(PrintSucceeded(command, {}, 'invalid pulse count parameter', -1));
          }
          break;
        default:
          resolve(PrintSucceeded(command, params, 'unknown command', -1));
      }
    });
  }
