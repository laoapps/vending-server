...
export class CreateLockerWalletFunc {
    ...
    public Init(params: any): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            ...
            try {
                ...
                console.log(`create vending wallet`, 1);

                this.InitParams(params);

                console.log(`create vending wallet`, 2);

                const ValidateParams = this.ValidateParams();
                if (ValidateParams != IENMessage.success) throw new Error(ValidateParams);

                console.log(`create vending wallet`, 3);

                const FindMerchant = await this.FindMerchant();
                if (FindMerchant != IENMessage.success) throw new Error(FindMerchant);

                console.log(`create vending wallet`, 4);
                ...
                const CreateLockerWallet = await this.CreateLockerWallet();
                if (CreateLockerWallet != IENMessage.success) throw new Error(CreateLockerWallet);

                console.log(`create vending wallet`, 5);
                ...
                console.log(`create vending wallet`, 6);
    ...
    private ValidateParams(): string {
        if (!(this.ownerUuid && this.machineClientId && this.username && this.platform)) return IENMessage.parametersEmpty;
    ...
    private FindVendingLimiter(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                if (run != null) throw new Error(IENMessage.vendingWalletHasAlreadyExisted);
    ...
    private CreateLockerWallet(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
    ...
    private UpdateMerchant(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                const run = await vendingWallet.update({ passkeys: this.passkeys }, { where: { uuid: this.lockeruuid }, transaction: this.transaction });
