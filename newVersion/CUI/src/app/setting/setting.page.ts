...
import { VendingMachineSerial_VMC } from '../services/VendingMachineSerial_VMC.service';
...
export class SettingPage implements OnInit, OnDestroy {
  ...
  motor = 0;
  log = '';
  vmc = new VendingMachineSerial_VMC();
  constructor(
  ...
  ngOnDestroy(): void {
    ...
    if (this.vmc) {
      this.vmc.removeAllListeners();
      this.vmc.disconnect();
    }
  }
  TESTDROP() {
    this.vmc.vendProduct(this.motor);
    this.log += '\n vendProduct', this.motor;
    this.vmc.on('data', (event) => {
