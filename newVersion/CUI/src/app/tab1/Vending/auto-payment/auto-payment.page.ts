...
export class AutoPaymentPage implements OnInit, OnDestroy {
  ...
  questionIcon: string = `../../../../assets/logo/loading.png`;
      ...
      try {
        ...
        this.countdownBillTimer = setInterval(async () => {
          ...
          if (this.countdownBill <= 0) {
            ...
            if (AutoPaymentPage.qrimgElement) AutoPaymentPage.qrimgElement.src = this.questionIcon;
  ...
  private Payment(): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        if (run.message != IENMessage.success) throw new Error(run);
        console.log('run QR Code IS :', run);
