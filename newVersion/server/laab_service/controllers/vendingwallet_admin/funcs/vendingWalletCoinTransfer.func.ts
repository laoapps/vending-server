...
export class VendingWalletCoinTransferFunc {
    ...
    private ValidateParams(): string {
        if (!(this.ownerUuid && this.machineClientId && this.coinListId && this.coinCode && this.sender && this.receiver && this.amount && this.description && this.limitBlock && this.coinListId && this.coinCode)) return IENMessage.parametersEmpty;
    ...
    private FindSender(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                const run = await vendingWallet.findOne({ where: { ownerUuid: this.ownerUuid, machineClientId: this.machineClientId, walletType: this.walletType } });
