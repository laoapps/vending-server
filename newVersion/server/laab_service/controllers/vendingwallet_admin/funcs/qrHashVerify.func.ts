...
import { IENMessage, LAAB_FindMyWallet, LAAB_QRHashVerify, LAAB_Register2, translateUToSU } from "../../../../services/laab.service";
import { IVendingWalletType } from "../../../models/base.model";
import { vendingWallet } from "../../../../entities";

export class QRHashVerifyFunc {
    ...
    private ValidateParams(): string {
        if (!(this.ownerUuid && this.sender && this.hashM && this.info && this.QRMode)) return IENMessage.parametersEmpty;
    ...
    private Verify(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                const run = await axios.post(LAAB_QRHashVerify, params);
                console.log(`rrrun`, run.data);
