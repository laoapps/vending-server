...
import { LoadProductListProcess } from '../products/processes/loadProductList.process';
import { LoadProductImagesProcess } from './processes/loadProductimages.process';
...
export class ImagesproductPage implements OnInit {
  private loadProductImagesProcess: LoadProductImagesProcess;
  ...
  loadProduct(): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        // }
        // const run = await this.loadProductImagesProcess.Init(params);
        // if (run.message != IENMessage.success) throw new Error(run);

        this.apiService.listProductImages().subscribe(run => {
          console.log('run', run.data);
          this._l.push(...run.data);
      ...
      ro?.onDidDismiss().then(r => {
        ...
        if (r.data.s) {
          ...
          this.filemanagerAPIService.writeFile(formfile).subscribe(r_writeFile => {
            ...
            this.apiService.addProductImages(r.data.s)?.subscribe(rx => {
              if (rx.status != 1) {
                this.filemanagerAPIService.cancelWriteFile({ uuid: fileuuid }).subscribe(r_cancelWriteFile => {
                  if (r_cancelWriteFile.status != 1) {
                    this.apiService.simpleMessage(IENMessage.cancelAndWriteFileFail);
  ...
  async addProduct(data: any) {
    ...
    try {
      const result = await this.apiService.presentAddProductDialog(data?.name, data?.price);
      if (result) {
