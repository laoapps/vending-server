...
class ADH814Protocol {
    ...
    parseResponse(address, command, buffer) {
        ...
        if ((command === 0xA5 || command === 0x21 || command === 0x35 || command === 0xA4) && data[0] !== 0) {
            console.log(`Command error: Code ${data[0]} - ${data[0] === 1 ? 'Invalid parameter' :
...
async function testMotorWithConfig() {
    ...
    try {
        ...
        setInterval(async () => {
            await requestID(protocol);
            // await setTemp(protocol); 
            await retry(protocol, address, 0x00); // Start motor 0x00
        }, 9000);
...
async function requestID(protocol,address=0x01) {
...
async function retry(protocol,address=0x01,motorNumber=0x00) {
    await polling(protocol, address);
    ...
    console.log('Start Motor Response:', motorResponse); // feedback status to client
    await protocol.acknowledgeResult(address);
}
async function setTemp(protocol,address=0x01) {
    await polling(protocol, address);
...
async function polling (protocol,address=0x01) {
    let statusResponse;
    console.log('Polling status...');
    while (statusResponse?.statusDetails?.status !== 2 && !statusResponse) { // 5 polls = 1.5 seconds
        ...
        console.log('retry Status Response:', statusResponse);
