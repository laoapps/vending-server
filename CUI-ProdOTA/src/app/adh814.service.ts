...
export class ADH814Service implements ISerialService {
  ...
  private async setupDevice(): Promise<void> {
    ...
    try {
      ...
      this.addLogMessage(this.log, 'ADH814Protocol setup completed successfully');
      return;
    } catch (err) {
  ...
  private startPolling(address: number, interval: number = 300): void {
    ...
    this.pollInterval = setInterval(() => {
      ...
      this.adh814.pollStatus(address, (response) => {
        ...
        this.machinestatus.data = JSON.stringify(result?.data);
  ...
  initializeSerialPort(
    ...
    return new Promise<string>(async (resolve, reject) => {
      ...
      try {
        ...
        await new Promise(resolve => setTimeout(resolve, 3000)); // Wait before setting temperature
