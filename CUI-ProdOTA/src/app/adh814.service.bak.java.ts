    ...
    return new Promise<IResModel>((resolve, reject) => {
      switch (command) {
        ...
        case EMACHINE_COMMAND.shippingcontrol:
          this.commandADH814('A5', params).then(resolve).catch(reject);
          break;
        case EMACHINE_COMMAND.START_MOTOR_MERGED:
          this.commandADH814('B5', params).then(resolve).catch(reject);
          break;
        case EMACHINE_COMMAND.CLEAR_RESULT:
          this.commandADH814('A6', params).then(resolve).catch(reject);
          break;
        case EMACHINE_COMMAND.QUERY_SWAP:
          this.commandADH814('34', params).then(resolve).catch(reject);
          break;
        case EMACHINE_COMMAND.SET_SWAP:
          this.commandADH814('35', params).then(resolve).catch(reject);
          break;
