...
export class LiveupdateService {
  ...
  async isNewBundleAvailable(latestBundleId: string): Promise<boolean> {
    try {
      ...
      return latestBundleId !== bundleIds[0]; // Compare with the first bundle ID
    } catch (error) {
  ...
  async getCurrentBundleId(): Promise<string | null> {
    try {
      const { bundleIds } = await LiveUpdate.getBundles();
      return bundleIds.length > 0 ? bundleIds[0] : null; // Return the first bundle ID
    } catch (error) {
  ...
  async verifyBundle() {
    try {
      ...
      return bundleIds.length > 0 ? bundleIds[0] : null; // Return the first bundle ID if available
    } catch (error) {
