...
import { IENMessage, LAAB_pathCounterIssued, LAAB_pathLogin } from '../services/laab.service';
export class Laab implements IBaseClass {
...
class Login {
    ...
    public Init(params: any): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                if (Validation != IENMessage.success) throw new Error(Validation);
    ...
    private Validation(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                const run = await axios.post(LAAB_pathLogin, params);
                if (run.data.status != 1) return resolve(run.data.message);

                this.token = run.data.info.token;
...
class CounterIssued {
    ...
    private ValidateParams(): string {
        if (!(this.receiver && this.amount)) return IENMessage.parametersEmpty;
        if (this.amount < 1 || this.amount > 10000000) return IENMessage.invalidAmount; 
        return IENMessage.success;
    }
    ...
    private Validation(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                const headers: any = { token: this.token }
                const run = await axios.post(LAAB_pathCounterIssued, params, { headers: headers });
                if (run.data.status != 1) return resolve(run.data.message);
