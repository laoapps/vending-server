...
import {
  APIAdminAccess,
  IENMessage,
  IFranchiseStockSignature,
  LAAB_CoinTransfer,
} from "../services/laab.service";
...
export class InventoryZDM8 implements IBaseClass {
  ...
  pathMMoneyConfirm =
    "https://api.mmoney.la/ewallet-ltc-api/cash-management/confirm-cash-in.service";
  pathMMoneyInquiry =
    "https://api.mmoney.la/ewallet-ltc-api/cash-management/inquiry-cash-in.service";
  pathMMoneyRequest =
    "https://api.mmoney.la/ewallet-ltc-api/cash-management/request-cash-in.service";
  ...
  checkMachineDisabled(req: Request, res: Response, next: NextFunction) {
    this.isMachineDisabled(res.locals["machineId"].machineId)
      ...
      .catch((e) => {
        return res.send(
          PrintError("checkMachineDisabled", e, EMessage.error, null)
        );
      });
  }
    ...
    try {
          ...
          if (d.command == EClientCommand.confirmMMoney) {
            ...
            if (!loggedin) throw new Error(EMessage.notloggedinyet);
              ...
              sEnt
                ...
                .then((r) => {
                  res.send(
                    PrintSucceeded("listSale", r, EMessage.succeeded, null)
                  );
                })
                .catch((e) => {
              ...
              const transactionID =
                String(mId.substring(mId.length - 10)) + (x + "").substring(2);
              ...
              ent.create(bill).then((r) => {
                ...
                res.send(
                  PrintSucceeded(d.command, r, EMessage.succeeded, null)
                );
              });
            } else {
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            if (mx) {
              if (moment().diff(moment(mx.t), "milliseconds") < 3000)
                return res.send(
                  PrintError(
                    "retryProcessBill",
                    [],
                    EMessage.error + " too fast",
                    returnLog(req, res, true)
                  )
                );
              mx.t = moment.now();
      ...
      router.get(this.path + "/getOnlineMachines", async (req, res) => {
        try {
          ...
          console.log(error);
          res.send(
            PrintError("init", error, EMessage.error, returnLog(req, res, true))
          );
        }
      });
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            const isActive = req.query["isActive"] + "" == "no" ? false : true;
            console.log(
              'req.query["isActive"]',
              !req.query["isActive"],
              req.query["isActive"],
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            if (!(ownerUuid && o.machineId && o.cloneMachineId))
              return res.send(
                PrintError(
                  "cloneSale",
                  [],
                  EMessage.parametersEmpty,
                  returnLog(req, res, true)
                )
              );
            const sEnt = VendingMachineSaleFactory(
            ...
            this.machineClientlist
              ...
              .then((r_findMachine) => {
                ...
                this.machineClientlist
                  .findOne({ where: { machineId: o.cloneMachineId } })
                  .then((r_findCloneMachine) => {
                    if (r_findCloneMachine == null)
                      return res.send(
                        PrintError(
                          "cloneSale",
                          [],
                          EMessage.notfoundCloneMachine,
                          returnLog(req, res, true)
                        )
                      );
                    sEnt
                      .findAndCountAll({ where: { machineId: o.machineId } })
                      .then((r_findMachineStock) => {
                        sEnt
                          .findAndCountAll({
                            where: { machineId: o.cloneMachineId },
                          })
                          .then((r_findCloneMachineStock) => {
                            if (r_findCloneMachineStock.count == 0)
                              return res.send(
                                PrintError(
                                  "cloneSale",
                                  [],
                                  EMessage.notFoundSaleForClone,
                                  returnLog(req, res, true)
                                )
                              );

                            let list: Array<any> = r_findMachineStock.rows;
                            let cloneList: Array<any> =
                              r_findCloneMachineStock.rows;
                            ...
                            sEnt
                              ...
                              .then((r_clonestock) => {
                                ...
                                for (let i = 0; i < models.length; i++) {
                                  ...
                                  models[i].createdAt =
                                    r_clonestock[i].createdAt;
                                  models[i].updatedAt =
                                    r_clonestock[i].updatedAt;
                                }
                                const loggg = [
                                  r_clonestock,
                                  list,
          ...
          this.machineClientlist
            ...
            .then((r) => {
              ...
              pEnt.findByPk(o.stock.id).then((p) => {
                ...
                sEnt
                  .findOne({
                    where: { position: o.position, machineId: o.machineId },
                  })
                  .then((rx) => {
                    if (rx)
                      return res.send(
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            if (
              (ads.machines != undefined &&
                Object.entries(ads.machines).length == 0) ||
              (ads.adsMedia != undefined &&
                Object.entries(ads.adsMedia).length == 0)
            ) {
              return res.send(
                PrintError(
                  "addAds",
                  e,
                  EMessage.parametersEmpty + "2",
                  returnLog(req, res, true)
                )
              );
            }
            ...
            machineClientIDEntity
              .findAll({ where: { machineId: { [Op.in]: ads.machines } } })
              .then((run_machine) => {
                if (
                  run_machine == undefined ||
                  Object.entries(run_machine).length !=
                    Object.entries(ads.machines).length
                ) {
                  return res.send(
                    PrintError(
                      "addAds",
                      e,
                      EMessage.invalidMachine,
                      returnLog(req, res, true)
                    )
                  );
                }
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            adsEntity
              .findAll({
                where: { machines: { [Op.contains]: [machineId.machineId] } },
              })
              .then((r) => {
                const latest = r
                  .filter((v) => existIds.includes(v.id))
                  ?.sort(
                    (a: any, b: any) =>
                ...
                if (!latest)
                  return res.send(
                    PrintSucceeded(
                      "loadAds",
                      { deletingArray: existIds, newArray: r },
                      EMessage.succeeded,
                      returnLog(req, res)
                    )
                  );
                ...
                res.send(
                  PrintSucceeded(
                    "loadAds",
                    {
                      deletingArray: deletingArray.map((item) => item),
                      newArray,
                    },
                    EMessage.succeeded,
                    returnLog(req, res)
            ...
            console.log(error);
            res.send(
              PrintError("loadAds", error, EMessage.error, returnLog(req, res))
            );
          }
        }
      );
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            const calculate = laabHashService.CalculateHash(
              JSON.stringify(d.data)
            );
            console.log(`calculate der`, calculate);
            const sign = laabHashService.Sign(
              calculate,
              IFranchiseStockSignature.privatekey
            );
      ...
      router.post(
        ...
        (req, res) => {
          try {
            ...
            this.loadVendingMachineSaleBillReport(parmas)
              .then((run) => {
      ...
      router.post(
        ...
        async (req, res) => {
          try {
            ...
            if (
              o.machineId.length != 8 ||
              Number.isNaN(o.machineId) ||
              Number.isNaN(o.otp)
            )
              return res.send(
                PrintError(
                  "addMachine",
                  [],
                  EMessage.InvalidMachineIdOrOTP,
                  returnLog(req, res, true)
                )
              );
          ...
          try {
            ...
            this.machineClientlist
              ...
              .then(async (r) => {
                ...
                r.photo = o.photo
                  ? base64ToFile(o.photo)
                  : base64ToFile(r.photo);
                // r.changed('isActive',true);
          ...
          try {
            ...
            this.machineClientlist
              ...
              .then(async (r) => {
                ...
                o.data[0].allowVending = !o.data[0]?.allowVending
                  ? false
                  : true;
                ...
                if (!a) {
                  ...
                  a.allowVending = x;
                  (a.allowCashIn = y), (a.light = w);
                  a.highTemp = z;
                  a.lowTemp = u;
                  (a.limiter = l), (a.imei = t);
                  a.imgHeader = imgh;
                  a.imgFooter = imgf;
                  a.imgLogo = imgl;
                }
          ...
          .then((r) => {
            ...
            if (phoneNumber) {
              findUuidByPhoneNumberOnUserManager(phoneNumber).then((r) => {
                ...
                if (ownerUuid) {
                  subadminEntity
                    .findOne({ where: { data: { uuid }, ownerUuid } })
                    .then((subadmin) => {
                      if (subadmin != null) {
    ...
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        if (isNaN(cashInValue))
          return reject(new Error("Invalid Cash In Value"));
        // validate phonenumber for MMoney
        if (phonenumber?.length < 10 || isNaN(phonenumber) || phonenumber == "")
          return reject("Invalid Phonenumber");
        // find in redis
        ...
        if (d?.token) {
          ...
          func
            ...
            .then((run) => {
              ...
              machineCashoutMMoneyEntity
                .create(model)
                .then((run_createLAABLog) => {
                  console.log(`creditMachineMMoney`, 3, run_createLAABLog);
                  if (!run_createLAABLog)
                    return resolve(IENMessage.createLAABBillFail);
                  machineCashoutMMoneyEntity
                    .findOne({ where: { id: run_createLAABLog.id } })
                    .then((run_findbill) => {
                      console.log(`creditMachineMMoney`, 4, run_findbill);
                      if (run_findbill == null)
                        return resolve(IENMessage.notFoundBill);
                      this.refillMMoney(
                        phonenumber,
                        transactionID,
                        cashInValue,
                        ...
                        .then((refill) => {
                          ...
                          machineCashoutMMoneyEntity
                            .update(
                              { MMoney: refill },
                              { where: { id: run_createLAABLog.id } }
                            )
                            .then((run_createMMoneyLog) => {
                              console.log(
                                `creditMachineMMoney`,
                                6,
                                run_createMMoneyLog
                              );
                              if (!run_createMMoneyLog)
                                return resolve(IENMessage.createMMoneyBillFail);
                              model.MMoney = refill;
                          ...
                          axios
                            ...
                            .then((run_return) => {
                              ...
                              if (run_return.data.status != 1) {
                                ...
                                machineCashoutMMoneyEntity
                                  .update(
                                    { LAABReturn: run_return.data },
                                    { where: { id: run_createLAABLog.id } }
                                  )
                                  .then((run_createMMoneyLog) => {
                                    console.log(
                                      `save return bill br sum led`,
                                      run_return.data
                                    );
                                    if (!run_createMMoneyLog)
                                      return resolve(
                                        IENMessage.createMMoneyBillFail
                                      );
                              ...
                              machineCashoutMMoneyEntity
                                ...
                                .then((run_createMMoneyLog) => {
                                  ...
                                  writeMachineBalance(
                                    ifError.machineId,
                                    ifError.vendingBalance
                                  );
                                  resolve(error.message);
                                })
                                .catch((error) =>
                                  resolve(
                                    `update bill return br sum led 001` +
                                      error.message
                                  )
                                );
                            })
                            .catch((error) => {
                              ...
                              machineCashoutMMoneyEntity
                                ...
                                .catch((error) =>
                                  resolve(
                                    `update bill return br sum led 002` +
                                      error.message
                                  )
                                );
                            });
          ...
          ws.send(
            JSON.stringify(
              PrintError(d.command, [], EMessage.MachineIdNotFound, null)
            )
          );
  ...
  async creditMachine(d: IReqModel) {
    try {
      ...
      // find in redis
      let machineId = this.ssocket.findMachineIdToken(d.token);
      let ack = await readACKConfirmCashIn(
        machineId.machineId + "" + d.transactionID
      );
      ...
      if (ack == "yes") {
        ...
        readMachineSetting(machineId.machineId).then(async (r) => {
          ...
          if (r) {
            try {
              ...
              console.log("error parsing setting 2", error);
              (setting.allowVending = true), (setting.allowCashIn = true);
              setting.lowTemp = 5;
              setting.highTemp = 10;
              setting.light = true;
              setting.limiter = 100000;
              setting.imei = "";
            }
          }
          const balance = await readMerchantLimiterBalance(machineId.ownerUuid);
          ...
          that.ssocket.updateBalance(machineId.machineId, {
            balance: balance || 0,
            limiter: setting.limiter,
            setting,
            confirmCredit: true,
        ...
        if (d?.token) {
          ...
          if (bn == undefined || Object.entries(bn).length == 0) {
            ws?.send(
              JSON.stringify(
                PrintError(d.command, [], EMessage.invalidBankNote, null)
              )
            );
            return;
          }
          ...
          func
            ...
            .then((run) => {
              ...
              readMachineSetting(machineId.machineId).then(async (r) => {
                ...
                const balance = await readMerchantLimiterBalance(
                  machineId.ownerUuid
                );
                // const limiter = await readMachineLimiter(machineId.machineId);
                ...
                that.ssocket.updateBalance(machineId.machineId, {
                  ...
                  setting,
                  confirmCredit: true,
                  transactionID: bsi.transactionID,
                });
                ws?.send(
                  JSON.stringify(
                    PrintSucceeded(d.command, res, EMessage.succeeded, null)
            ...
            .catch((error) => {
              ...
              ws?.send(
                JSON.stringify(PrintError(d.command, [], error.message, null))
              );
            });
  ...
  init() {
    ...
    return new Promise<Array<IMachineClientID>>((resolve, reject) => {
      this.machineClientlist.sync().then((r) => {
        this.machineClientlist
          .findAll({ attributes: { exclude: ["photo"] } })
          .then((rx) => {
      ...
      this.ssocket.onMachineResponse((re: IReqModel) => {
        if (re.transactionID == -52) {
          ...
          const ws = this.wsClient.find(
            (v) => v["machineId"] == machineId.machineId
          );
          const wsAdmins = this.wsClient.find((v) =>
            v["myMachineId"]?.includes(machineId.machineId)
          );
  ...
  loadBillCash(machineId: string, transactionID: number) {
    return new Promise<Array<IBillCashIn>>((resolve, reject) => {
      try {
        this.billCashEnt
          .findAll({ where: { transactionID, machineId } })
          .then((r) => {
            resolve(r);
          })
          .catch((e) => {
  ...
  loadBadBillCash(machineId: string, transactionID: number) {
    return new Promise<Array<IBillCashIn>>((resolve, reject) => {
      try {
        this.badBillCashEnt
          .findAll({ where: { transactionID, machineId } })
  ...
  generateBillMMoneyPro(
    ...
    return new Promise<IMMoneyGenerateQRRes>((resolve, reject) => {
      ...
      axios
        .post<IMMoneyGenerateQRRes>("https://qr.mmoney.la/pro/GenerateQR", qr, {
          headers: {
            "lmm-key":
              "va157f35a50374ba3a07a5cfa1e7fd5d90e612fb50e3bca31661bf568dcaa5c17",
          },
  ...
  processRefillMmoney(
    ...
    return new Promise<any>((resolve, reject) => {
      this.requestMmoneyCashin(msisdn, transID, value, accessToken, remark)
        .then((r) => {
  ...
  confirmMmoneyCashin(
    ...
    const url = this.production
      ? this.pathMMoneyConfirm
      : "http://115.84.121.101:31153/ewallet-ltc-api/cash-management/confirm-cash-in.service";
    return new Promise<any>((resolve, reject) => {
      const data = {
  ...
  loginCashInMmoney() {
    const url = this.production
      ? this.pathMMoneyLogin
      : "http://115.84.121.101:31153/ewallet-ltc-api/oauth/token.service";
    return new Promise<IMMoneyLoginCashin>((resolve, reject) => {
      ...
      params.append(
        "username",
        this.production ? this.CashInMMoneMMoneyUsername : "Dokbuakham"
      );
      params.append(
        "password",
        this.production ? this.CashInMMoneMMoneyPassword : "Ko8-En6;"
      );
  ...
  requestMmoneyCashin(
    ...
    value,
    accessToken: string,
    remark = this.production
      ? this.CashInMMoneMMoneyName
      : "Test Dorkbouakham Cash-In"
  ) {
    const url = this.production
      ? this.pathMMoneyRequest
      : "http://115.84.121.101:31153/ewallet-ltc-api/cash-management/request-cash-in.service";
    return new Promise<IMMoneyRequestRes>((resolve, reject) => {
      ...
      data.transCheckSum = this.checkSum(
        data.toAccountRef,
        value,
        data.transRemark,
        data.transRefCol1,
                ...
                readMachineBalance(ws["machineId"]).then(async (r) => {
                  ...
                  try {
                    ...
                    if (!x) {
                      ...
                      setting.lowTemp = 5;
                      setting.highTemp = 10;
                      (setting.light = true), (setting.imei = "");
                    } else {
                    ...
                    mymlimiterbalance =
                      (await readMerchantLimiterBalance(ws["ownerUuid"])) ||
                      "0";
                    ...
                    for (let index = 0; index < mArray?.length; index++) {
                      ...
                      mymstatus.push({
                        machineId: element,
                        mstatus: await readMachineStatus(element),
                      });

                      const msetting = JSON.parse(
                        await readMachineSetting(element)
                      );
                      mymsetting.push({ msetting, machineId: element });

                      const mb = JSON.parse(
                        (await readMachineBalance(element)) || "0"
                      );
                      mymmachinebalance.push({
                        machineId: element,
                        balance: mb,
                      });
  ...
  loadVendingMachineSaleBillReport(
    params: ILoadVendingMachineSaleBillReport
  ): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      try {
...
export class loadVendingMachineSaleBillReport {
  ...
  public Init(params: ILoadVendingMachineSaleBillReport): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        if (ValidateParams != IENMessage.success)
          throw new Error(ValidateParams);

        const ValidateBeginDate = this.ValidateBeginDate();
        if (ValidateBeginDate != IENMessage.success)
          throw new Error(ValidateBeginDate);
  ...
  private ValidateParams(): string {
    if (!(this.ownerUuid && this.fromDate && this.toDate && this.machineId))
  ...
  private ValidateBeginDate(): string {
    ...
    if (this.parseFromDate == this.parseToDate) {
      if (this.parseFromDate > this.currentdate)
        return IENMessage.invalidFromDate;
    } else {
      if (this.parseFromDate > this.parseToDate)
        return IENMessage.invalidFromDate;
      if (this.parseToDate > this.currentdate)
        return IENMessage.invalidateToDate;
    }
  ...
  private Connection(): void {
    this.vendingMachineBillEntity = VendingMachineBillFactory(
      EEntity.vendingmachinebill + "_" + this.ownerUuid,
      dbConnection
    );
  }
  ...
  private Report(): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      try {
        const run = await this.vendingMachineBillEntity.findAndCountAll(
          this.condition
        );
        this.response = {
          rows: run.rows,
