...
export class LoadVendingMachineSaleBillReportProcess {
        ...
        return new Promise<any>(async (resolve, reject) => {
            try {
                ...
                this.apiService.loadVendingMachineSaleBillReport(params).subscribe(r => {
                    ...
                    const stock = list.map(obj => obj.vendingsales.map(item => { return { machineId: obj.machineId, time: obj.updatedAt, stock: item.stock, paymentref: obj.paymentref, paymentstatus: obj.paymentstatus, dropAt: item.dropAt, createdAt: obj.createdAt, position: item.position } }));
    ...
    private FetchOrder(): void {
        ...
        allsale = cloneSaleDetailList.map(item => { return { stock: item.stock, machineId: item.machineId, paymentref: item.paymentref, paymentstatus: item.paymentstatus, dropAt: item.dropAt, createdAt: item.createdAt, position: item.position } });
