...
export class LoadMachineListProcess {
    ...
    public Init(params: any): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {
                ...
                const LoadMachineList = await this.LoadMachineList();
                if (LoadMachineList != IENMessage.success) throw new Error(LoadMachineList);
    ...
    private LoadMachineList(): Promise<any> {
        return new Promise<any> (async (resolve, reject) => {
            try {

                this.apiService.listMachine().subscribe(r => {
                    const response: any = r;
                    if (response.status != 1) return resolve(IENMessage.loadMachineListFail);
