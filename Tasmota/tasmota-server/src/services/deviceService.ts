...
const controlDeviceSchema = z
  ...
  .refine(
    (data) =>
      !data.conditionType || (data.conditionType && data.conditionValue),
...
export class DeviceService {
  ...
  static async getDevicesBy(data: any): Promise<Device[]> {
    if(data?.id){
      return models.Device.findAll({where:{id:data?.id}});
    }
      ...
      if (data?.dtype) {
        return models.Device.findAll({
          where: {
            ...
            description: { dtype: data.dtype },
          },
        });
      } else {
        return models.Device.findAll({
          where: { ownerId: data.ownerId, groupId: data.groupId },
        });
      }
    } else {
      if (data?.dtype) {
  ...
  static async getDevices(user: User, dtype: string): Promise<Device[]> {
    if (user.role === "admin") {
      return dtype
        ? models.Device.findAll({ where: { description: { dtype } } })
  ...
  static async updateDevice(
    ...
    const device = await models.Device.findOne({
      where: { id, ownerId: owner.dataValues.id },
    });
  ...
  static async controlDevice(
    ...
    try {
      const { command, relay, conditionType, conditionValue } =
        controlDeviceSchema.parse(input);
      const device = await models.Device.findByPk(deviceId);

      if (!device) throw new Error("Device not found");
      console.log(
        `Controlling device with ID: ${deviceId}, Command: ${command}, Relay: ${relay}`
      );

      // Clear existing rule and timer
      await publishMqttMessage(`cmnd/${device.dataValues.tasmotaId}/Rule1`, "");
      await publishMqttMessage(
        `cmnd/${device.dataValues.tasmotaId}/Timer1`,
        ""
      );
      ...
      if (conditionType === "energy_consumption" && conditionValue) {
        await publishMqttMessage(
          `cmnd/${device.dataValues.tasmotaId}/EnergyReset`,
          "0"
        );
        const rule = `ON Energy#Total>${conditionValue} DO Power${relay} OFF ENDON`;
        await publishMqttMessage(
          `cmnd/${device.dataValues.tasmotaId}/Rule1`,
          rule
        );
        await publishMqttMessage(
          `cmnd/${device.dataValues.tasmotaId}/Rule1`,
      ...
      console.log(
        `Sending MQTT command to ${mqttTopic} with payload: ${command}`
      );
