...
import models, { sequelize } from './models'; // ✅ Only use initialized models from here
import redis from './config/redis';
import { Op, WhereOptions } from 'sequelize';

import { userClients, adminClients, ownerClients, notifyStakeholders } from './services/wsService';
import { findRealDB } from './services/userManagerService';
import { getDeviceFromCache, publishMqttMessage } from './services/mqttService';
import { startDeviceMonitoring } from './controllers/monitorOrderController';
  ...
  try {
    ...
    for (const key of orderKeys) {
      ...
      if (order.completedTime) {
        const device = await order.getDevice();
        if (device) {
          await publishMqttMessage(`cmnd/${device.tasmotaId}/POWER${orderData.relay || 1}`, 'OFF');
          await publishMqttMessage(`cmnd/${device.tasmotaId}/Rule1`, '');
          await publishMqttMessage(`cmnd/${device.tasmotaId}/Timer1`, '');
        }
        await redis.del(key);
        await notifyStakeholders(order, 'Order terminated due to completed state on server restart.');
        continue;
      ...
      // Reconfigure energy-based automation
      if (orderData.conditionType === 'energy_consumption') {
        const device = await order.getDevice();
        if (device) {
          const relay = orderData.relay || 1;
          const threshold = orderData.conditionValue;
          await publishMqttMessage(`cmnd/${device.tasmotaId}/Rule1`, '');
          await publishMqttMessage(`cmnd/${device.tasmotaId}/Timer1`, '');
          await publishMqttMessage(`cmnd/${device.tasmotaId}/EnergyReset`, '0');
          const rule = `ON Energy#Total>${threshold} DO Power${relay} OFF ENDON`;
          await publishMqttMessage(`cmnd/${device.tasmotaId}/Rule1`, rule);
          await publishMqttMessage(`cmnd/${device.tasmotaId}/Rule1`, '1');
        }
      }
    }
...
async function startServer() {
  try {
    ...
    // Scheduled cleanup and condition-based logic
    cron.schedule('* * * * *', async () => {
      try {
        const orderKeys = await redis.keys('activeOrder:*');
        const ordersData = await Promise.all(
        ...
        for (const order of ordersData) {
          ...
          if (order.data.conditionType === 'time_duration') {
            const elapsedMinutes = (Date.now() - order.data.startedTime) / (60 * 1000);
            if (elapsedMinutes >= order.data.conditionValue) {
              await publishMqttMessage(`cmnd/${order.data.tasmotaId}/POWER${order.data.relay || 1}`, 'OFF');
            ...
            if (energy >= order.data.conditionValue) {
              ...
              if (ord) {
                ...
                await ord.save();
                await redis.del(key);
                await notifyStakeholders(ord, 'Order completed due to energy consumption limit.');
              }
            }
          }
        }

        // Cleanup unpaid orders older than 1 hour
        const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
        const whereCondition: WhereOptions = {
          paidTime: { [Op.is]: null },
          createdAt: { [Op.lte]: oneHourAgo },
        };
        const deletedCount = await models.Order.destroy({ where: whereCondition });
        if (deletedCount > 0) {
          console.log(`🧹 Deleted ${deletedCount} unpaid order(s)`);
        }
      } catch (error) {
